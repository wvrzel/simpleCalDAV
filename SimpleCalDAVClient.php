...
class SimpleCalDAVClient
{
    ...
    function connect($url, $user, $pass)
    {
        ...
        if ($client->GetHttpResultCode() != '200') {
            ...
            {
                throw new CalDAVException('Recieved unknown HTTP status while checking the connection after establishing it',
                    $client);
            }
        }
    ...
    function create($cal)
    {
        ...
        if ($this->client->GetHttpResultCode() == '200') {
            ...
            if ($this->client->GetHttpResultCode() == '404') {
                ;
            } else {
        ...
        if ($this->client->GetHttpResultCode() != '201') {
            ...
            {
                throw new CalDAVException($this->url . $uid . '.ics already existed. Entry has been overwritten.',
                    $this->client);
            } else // Unknown status
            {
                throw new CalDAVException('Recieved unknown HTTP status', $this->client);
    ...
    function getEvents($start = null, $end = null)
    {
        ...
        if ((isset($start) and !preg_match('#^\d\d\d\d\d\d\d\dT\d\d\d\d\d\dZ$#', $start, $matches))
            ...
            trigger_error('$start or $end are in the wrong format. They must have the format yyyymmddThhmmssZ and should be in GMT',
                E_USER_ERROR);
        }
